...
$select_sth = $dbh->prepare(
  'SELECT bbs_entries.*, 
   (SELECT name FROM users WHERE users.id = bbs_entries.user_id) AS user_name,
   (SELECT icon_filename FROM users WHERE users.id = bbs_entries.user_id) AS icon_filename
  FROM bbs_entries
  ORDER BY bbs_entries.created_at DESC'
);
$select_sth->execute();
...
?>
<?php if(empty($_SESSION['login_user_id'])): ?>
  投稿するには<a href="./1022/login.php">ログイン</a>が必要です。
<?php else: ?>
  <!-- フォームのPOST先はこのファイル自身にする -->
  <form method="POST" action="./bbs.php">
    <textarea name="body" required></textarea>
  ...
  <dl style="margin-bottom: 1em; padding-bottom: 1em; border-bottom: 1px solid #ccc;">
    ...
    <dd>
      ...
      <img src="/image/<?= htmlspecialchars($entry['icon_filename']) ?>"
           style="width:3em; height:3em; border-radius:50%; object-fit:cover; vertical-align:middle; margin-right:0.5em;">
      <?php endif; ?>
      <?= htmlspecialchars($entry['user_name']) ?>
      (ID: <?= htmlspecialchars($entry['user_id']) ?>)
      </a>
